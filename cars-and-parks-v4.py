...
def find_closest_centroid(car_centroids, car_park_centroid, search_radius=100, used_centroids=set()):
    """
    Find the closest car centroid to a given car park centroid within a search radius.
    
    Args:
    car_centroids (list of tuples): List of (x, y) coordinates of car centroids.
    car_park_centroid (tuple): (x, y) coordinates of the car park centroid.
    search_radius (int): The radius within which to search for the closest centroid.
    used_centroids (set): Set of centroids that have already been used.
    
    Returns:
    tuple: The closest car centroid to the car park centroid.
    """
    cx, cy = car_park_centroid
    closest_centroid = None
    min_distance = float('inf')
    
    for car_centroid in car_centroids:
        if car_centroid in used_centroids:
            continue
        car_cx, car_cy = car_centroid
        if abs(car_cx - cx) <= search_radius and abs(car_cy - cy) <= search_radius:
    ...
    if success:
        ...
        for carPark in carParks:
            ...
            if M['m00'] != 0:
                ...
                # Find the closest car centroid to the car park centroid within the search radius
                closest_car_centroid = find_closest_centroid(car_centroids, car_park_centroid, search_radius=100, used_centroids=used_centroids)
                if closest_car_centroid:
                    cv2.line(annotated_frame, car_park_centroid, closest_car_centroid, (255, 255, 0), 2)
                    used_centroids.add(closest_car_centroid)
