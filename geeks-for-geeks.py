...
while True:
    ...
    # Run YOLO with mixed precision (FP16) and only detect cars (class_id=2)
    detections = model(frame_resized, classes=[CAR_CLASS_ID], half=True)[0]  # Limit detection to "cars"
    results = []
    ...
    tracks = tracker.update_tracks(results, frame=frame_resized)
    for track in tracks:
        ...
        cv2.rectangle(frame_resized, (xmin, ymin), (xmax, ymax), GREEN, 2)
        cv2.rectangle(frame_resized, (xmin, ymin - 20), (xmin + 20, ymin), GREEN, -1)
        cv2.putText(frame_resized, str(track_id), (xmin + 5, ymin - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 2)

    end = datetime.datetime.now()  # Record the end time
    fps = f"FPS: {1 / (end - start).total_seconds():.2f}"  # Calculate and display the FPS
    cv2.putText(frame_resized, fps, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 8)
